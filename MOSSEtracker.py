...
def get_scale_sample(im, pos, base_target_size, scale_factors, cos_window, scale_model_size):
	...
	for i in range(n):
        ...
        x_start = max(0, np.floor(pos[1]) - np.floor(patch_size[1] / 2))
        y_start = max(0, np.floor(pos[0]) - np.floor(patch_size[0] / 2))
        x_stop = min(im.shape[1] - 1, np.floor(pos[1]) + patch_size[1] - np.floor(patch_size[1] / 2))
        y_stop = min(im.shape[0] - 1, np.floor(pos[0]) + patch_size[0] - np.floor(patch_size[0] / 2))
...
def get_features(im):
    ...
    if len(im.shape) == 2:
        f[:, :, 0] = im - 0.5
        f[1:-1, 1:-1, 1:] = pyhog.features_pedro(color.gray2rgb(im_patch), 1)[:, :, :27]
    else:
...
def get_sample(im, pos, base_target_size, scale_factor, cos_window):
    ...
    patch_size[0] = 2 if patch_size[0] < 1
    patch_size[1] = 2 if patch_size[1] < 1

    x_start = max(0, np.floor(pos[1]) - np.floor(patch_size[1] / 2))
    y_start = max(0, np.floor(pos[0]) - np.floor(patch_size[0] / 2))
    x_stop = min(w - 1, np.floor(pos[1]) + patch_size[1] - np.floor(patch_size[1] / 2))
    y_stop = min(h - 1, np.floor(pos[0]) + patch_size[0] - np.floor(patch_size[0] / 2))
    im_patch = im[y_start.astype(np.int32):y_stop.astype(np.int32),
                  x_start.astype(np.int32):x_stop.astype(np.int32)]
...
class Tracker(object):
    ...
    def __init__(self, **params):
        ...
        self.scale_sigma_factor = params.get('scale_sigma_factor', 0.0625)
        self.lbd = params.get('lambda', 0.01)
        self.lr = params.get('lr', 0.025)
        self.num_scales = params.get('num_scales', 5)
        self.scale_step = params.get('scale_step', 1.1)
    ...
    def initial(self, img, pos, target_size):
        ...
        self.search_size = np.floor(base_target_size * (1 + self.padding))
    ...
    def update(self, im, init_tracker=False):
        ...
        scale_sample = get_scale_sample(im, pos, self.base_target_size,
        	self.current_scale_factor * self.scale_factors, self.scale_window, self.scale_model_size)
        ...
        if self.hf_den is None or init_tracker:
            ...
            self.hf_den = (1 - self.learning_rate) * self.hf_den + self.learning_rate * new_hf_den
            self.hf_num = (1 - self.learning_rate) * self.hf_num + self.learning_rate * new_hf_num
    ...
    def get_scale_filter(self):
        ...
        scale_sigma = np.sqrt(self.num_scales) * self.scale_sigma _factor
...
if __name__ == '__main__':
    img = cv2.imread('./dog1/imgs/00001.jpg')
